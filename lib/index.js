...
/** Default alert thresholds (currency units); empty = no alerts. */
const DEFAULT_THRESHOLDS = []
/** Durable settings schema: ordered, selectable balance rows + refresh interval + alert thresholds. */
const SettingsSchema = z.object({
  ...
  thresholds: z.array(z.number().min(0)).default(DEFAULT_THRESHOLDS)
})
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const read = () => {
        ...
        const thresholds = section && Array.isArray(section.thresholds)
          ? section.thresholds.filter((value) => typeof value === 'number' && Number.isFinite(value) && value >= 0)
          : DEFAULT_THRESHOLDS
        return { fields, refreshSeconds, thresholds }
      }
      if (req.method === 'GET' || req.method === 'HEAD') {
        ...
        try {
          ...
          if (payload.thresholds !== undefined) {
            if (!Array.isArray(payload.thresholds)
              || payload.thresholds.some((value) => typeof value !== 'number' || !Number.isFinite(value) || value < 0)) {
              json(res, 400, { ok: false, error: 'thresholds must be an array of non-negative numbers' })
              return
            }
            patch.thresholds = payload.thresholds
          }
...
export { DEFAULT_FIELDS, DEFAULT_REFRESH_SECONDS, DEFAULT_THRESHOLDS, MIN_REFRESH_SECONDS, SETTINGS_NAMESPACE, apply, inject, name }
