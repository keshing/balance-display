...
/** Durable settings schema: ordered, selectable balance rows. */
    ...
    handler: async (req, res) => {
      ...
      if (req.method === 'POST') {
        try {
          ...
          const fields = payload.fields.filter((key) => typeof key === 'string' && VALID_FIELDS.includes(key))
          await settings.update(SETTINGS_NAMESPACE, { fields })
          json(res, 200, { ok: true, ...read() })
        } catch (error) {
...
export { DEFAULT_FIELDS, SETTINGS_NAMESPACE, apply, inject, name }
