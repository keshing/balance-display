window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const en = {
      ...
      'alert.below': 'Balance dropped below ',
    ...
    function apply(ctx) {
      ...
      // Evaluate the latest balance against given thresholds; enqueue a toast
      // for each threshold crossed downward. Called after every fetch.
      const evaluateAlerts = (amount, currency, thresholds) => {
        ...
        if (fired.length > 0) {
          setSnapshot({ alerts: [...snapshot.alerts, ...fired.map((threshold) => ({ id: 'a' + threshold + '-' + Date.now(), threshold, amount, currency }))] })
        }
      }
      ...
      function BalanceAlertStack(props) {
        ...
        react.useEffect(() => {
          if (view.alerts.length === 0) return
          dismissTimer.current = ctx.timeout(() => {
            setSnapshot({ alerts: snapshot.alerts.slice(1) })
          }, 6000)
          return () => {
            if (dismissTimer.current) dismissTimer.current()
          }
        }, [view.alerts.length])
      ...
      function BalanceSettingsSection(props) {
        ...
        const testAlert = () => {
          ...
          setSnapshot({
            alerts: [...snapshot.alerts, { id: 'test-' + Date.now(), threshold, amount, currency }]
          })
        }
