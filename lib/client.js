window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      const css = [
        '.dyn-balance-wrap{position:relative;display:inline-flex;align-items:center}',
        '.dyn-balance-cell{cursor:pointer;width:28px;height:28px;color:var(--dsw-alias-label-secondary);background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex;font-size:14px;user-select:none}',
        '.dyn-balance-cell:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(128,128,128,.12))}',
        '.dyn-balance-glyph{font-weight:700;font-size:15px;line-height:1}',
        '.dyn-balance-cell--error{color:var(--dsw-alias-state-error-primary)}',
        '.dyn-balance-cell--rail{width:36px;height:36px}',
      ...
      // Durable settings: ordered, selectable balance rows.
      ...
      const readFields = () => {
        ...
        fetch('/api/balance/settings', { cache: 'no-store' }).then((response) => {
          ...
          if (status !== 200 || payload === null || payload.ok === false) {
            setSnapshot({ fields: null })
            return
          }
          if (Array.isArray(payload.fields)) {
            setSnapshot({ fields: payload.fields })
          } else {
            setSnapshot({ fields: null })
          }
        }).catch(() => {
          if (alive) setSnapshot({ fields: null })
        })
        return () => { alive = false }
      }
      ...
      // Simple shared store: balance snapshot, popover open state, anchor rect, settings fields.
      ...
      // trigger readFields from the component).
      const refreshFields = () => { readFields() }
      ctx.effect(() => {
        refreshFields()
      ...
      function BalanceButton(props) {
        ...
        react.useEffect(() => ctx.interval(() => load(), 5 * 60 * 1000), [load])
        react.useEffect(() => {
          if (!view.open) return
          const onDown = (event) => {
      ...
      // The settings page: pick which rows show and in what order.
      function BalanceSettingsSection(props) {
        const t = props.t
        const view = useStore()
        react.useEffect(() => { readFields() }, [])
        const fields = view.fields === null ? DEFAULT_FIELDS : view.fields
        const setField = (key, enabled) => {
          const next = enabled
            ? [...fields, key]
            : fields.filter((k) => k !== key)
          setSnapshot({ fields: next })
          writeFields(next).then(() => readFields()).catch((error) => {
        ...
        for (const def of FIELD_DEFS) {
          if (fields.includes(def.key)) continue
