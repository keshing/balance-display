window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      const writeFields = (fields) => writeSettings({ fields })
      const writeRefresh = (refreshSeconds) => writeSettings({ refreshSeconds })
      ...
      // The settings page: pick which rows show, their order, and the refresh interval.
      function BalanceSettingsSection(props) {
        const t = props.t
        const view = useStore()
        react.useEffect(() => { readSettings() }, [])
        const fields = view.fields === null ? DEFAULT_FIELDS : view.fields
        // Refresh interval local draft: [hours, minutes, seconds] + error state.
        const [draft, setDraft] = react.useState(() => {
          const total = view.refreshSeconds === null ? DEFAULT_REFRESH_SECONDS : view.refreshSeconds
          return {
        ...
        react.useEffect(() => {
          if (view.refreshSeconds === null) return
          setDraft({
            h: String(Math.floor(view.refreshSeconds / 3600)),
            m: String(Math.floor((view.refreshSeconds % 3600) / 60)),
            s: String(view.refreshSeconds % 60)
          })
          setInvalid(false)
        }, [view.refreshSeconds])
        const applyRefresh = (next) => {
          setDraft(next)
          const h = parseInt(next.h, 10)
          const m = parseInt(next.m, 10)
          const s = parseInt(next.s, 10)
          const parts = [h, m, s]
          ...
          if (total < MIN_REFRESH_SECONDS) {
            setInvalid(true)
            return
          }
          setInvalid(false)
          setSnapshot({ refreshSeconds: total })
          writeRefresh(total).then(() => readSettings()).catch((error) => {
            console.error('[balance-display] refresh write failed:', error)
            readSettings()
          })
        }
        const setField = (key, enabled) => {
          ...
          writeFields(next).then(() => readSettings()).catch((error) => {
