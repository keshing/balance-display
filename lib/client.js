window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      'settings.reset': '重置',
      'settings.alert': '预警通知',
      'settings.alertHint': '余额低于阈值时，右下角会弹出一次提醒。余额回升到阈值以上后再次跌破会重新提醒。',
      'settings.alertAdd': '添加阈值',
      'settings.alertPlaceholder': '金额',
      'settings.alertInvalid': '阈值必须是大于等于 0 的数字。',
      'settings.alertTest': '测试提醒',
      'alert.title': '余额预警',
      'alert.below': '当前余额 ',
      'alert.current': '当前余额'
    }
    const en = {
      ...
      'settings.alert': 'Alert notifications',
      'settings.alertHint': 'A toast appears in the bottom-right when the balance drops below a threshold. It re-alerts after the balance rises above the threshold and falls again.',
      'settings.alertAdd': 'Add threshold',
      'settings.alertPlaceholder': 'Amount',
      'settings.alertInvalid': 'Thresholds must be numbers of at least 0.',
      'settings.alertTest': 'Test alert',
      'alert.title': 'Balance alert',
      'alert.below': 'Current balance ',
      'alert.current': 'Current balance'
    }
    ...
    // Currency symbol for display: known codes map to a glyph, unknown codes
    // fall back to the three-letter code with a trailing space.
    const SYMBOL = (currency) => {
      const code = String(currency || 'CNY')
      return SYMBOLS[code] !== undefined ? SYMBOLS[code] : code + ' '
    }
    ...
    function apply(ctx) {
      ...
      const css = [
        // Stack footer actions vertically (like VS Code's Activity Bar) so every
        // plugin's button gets its own row and none squeeze the others.
        '[class$="footerActions"]{flex-direction:column;align-items:stretch}',
        // In the collapsed rail, the shell centers footer actions; keep that
        // horizontal centering with the column direction.
        '[class$="collapsed"] [class$="footerActions"]{justify-content:flex-start;align-items:center}',
        '.dyn-balance-wrap{position:relative;display:flex;flex:none;width:100%;min-width:0;align-items:center}',
        ...
        '.dyn-balance-alert{position:absolute;right:16px;bottom:16px;z-index:10001;pointer-events:auto;max-width:320px;background:var(--dsw-alias-bg-overlay,#fff);border:1px solid var(--dsw-alias-border-l2,rgba(128,128,128,.35));border-radius:10px;padding:10px 26px 10px 12px;box-shadow:0 6px 20px rgba(0,0,0,.2);color:var(--dsw-alias-label-primary);font-size:13px;line-height:1.6}',
        'body[data-ds-dark-theme] .dyn-balance-alert{background:color-mix(in srgb,var(--dsw-alias-bg-overlay) 55%,var(--dsw-specific-sidebar-fill) 45%)}',
        '.dyn-balance-alert-title{font-weight:600;font-size:13px;color:var(--dsw-alias-state-warn-primary);margin-bottom:2px}',
        '.dyn-balance-alert-body{font-size:12px;color:var(--dsw-alias-label-secondary)}',
        '.dyn-balance-alert-close{position:absolute;top:6px;right:8px;border:none;background:0 0;color:var(--dsw-alias-label-tertiary);cursor:pointer;font-size:14px;line-height:1;padding:2px}',
        '.dyn-balance-alert-add-row{display:flex;align-items:center;gap:8px;margin-top:8px;flex-wrap:wrap}'
      ].join('\n')
      ctx.effect(() => {
      ...
      const readSettings = () => {
        ...
        fetch('/api/balance/settings', { cache: 'no-store' }).then((response) => {
          ...
          if (status !== 200 || payload === null || payload.ok === false) {
            setSnapshot({ fields: null, refreshSeconds: null, thresholds: null })
            return
          }
          setSnapshot({
            ...
            thresholds: Array.isArray(payload.thresholds) ? payload.thresholds.filter((v) => typeof v === 'number' && v >= 0) : null
          })
        }).catch(() => {
          if (alive) setSnapshot({ fields: null, refreshSeconds: null, thresholds: null })
        })
        return () => { alive = false }
      }
      ...
      // Simple shared store: balance snapshot, popover open state, anchor rect, settings, alerts.
      ...
      // Alert edge tracking: which thresholds the balance is currently ABOVE.
      // A threshold fires when the balance crosses from above it to at/below it.
      const aboveThresholds = /* @__PURE__ */ new Set()
      // Evaluate the latest balance against given thresholds; when multiple
      // thresholds fire at once, keep only the LOWEST (most critical) one, and
      // a newer alert replaces any older toast. Called after every fetch.
      const evaluateAlerts = (amount, currency, thresholds) => {
        const list = thresholds === null ? [] : thresholds
        const fired = []
        for (const threshold of list) {
          const above = amount > threshold
          if (above) {
            aboveThresholds.add(threshold)
          } else if (aboveThresholds.has(threshold)) {
            aboveThresholds.delete(threshold)
            fired.push(threshold)
          }
          // First evaluation with no prior state: only alert if we had seen it above.
          // If the balance is already below on the very first check, do not spam.
        }
        if (fired.length > 0) {
          const lowest = Math.min(...fired)
          setSnapshot({ alerts: [{ id: 'a' + lowest + '-' + Date.now(), threshold: lowest, amount, currency }] })
        }
      }
      ...
      function BalanceButton(props) {
        ...
        const load = react.useCallback(() => {
          ...
          fetch('/api/balance', { cache: 'no-store' }).then((response) => {
            ...
            evaluateAlerts(amount, currency, snapshot.thresholds)
          }).catch((error) => {
            if (!alive) return
      ...
      // Bottom-right alert toasts; each auto-dismisses after a few seconds.
      function BalanceAlertStack(props) {
        const t = props.t
        const view = useStore()
        const dismiss = (id) => {
          setSnapshot({ alerts: snapshot.alerts.filter((alert) => alert.id !== id) })
        }
        // Alerts persist until the user closes them (×) — no auto-dismiss.
        return react.createElement(react.Fragment, null,
          view.alerts.map((alert) => {
            const symbol = SYMBOL(alert.currency)
            return react.createElement('div', { className: 'dyn-balance-alert', key: alert.id },
              react.createElement('button', {
                type: 'button',
                className: 'dyn-balance-alert-close',
                onClick: () => dismiss(alert.id),
                'aria-label': 'close'
              }, '×'),
              react.createElement('div', { className: 'dyn-balance-alert-title' },
                t('alert.title') + ': ' + symbol + alert.threshold.toFixed(2)),
              react.createElement('div', { className: 'dyn-balance-alert-body' },
                t('alert.below') + symbol + alert.amount.toFixed(2)))
          }))
      }
      ...
      function BalanceSettingsSection(props) {
        ...
        const committedThresholds = view.thresholds === null ? [] : view.thresholds
        const splitRefresh = (total) => ({
          h: String(Math.floor(total / 3600)),
          m: String(Math.floor((total % 3600) / 60)),
          s: String(total % 60)
        })
        // Draft state: fields order + refresh h/m/s inputs + alert thresholds.
        ...
        const [draftThresholds, setDraftThresholds] = react.useState(committedThresholds)
        ...
        react.useEffect(() => {
          ...
          setDraftThresholds(committedThresholds)
          setInvalid(false)
        }, [view.fields, view.refreshSeconds, view.thresholds])
        const addThreshold = () => {
          const value = Number(thresholdInput)
          if (thresholdInput.trim() === '' || !Number.isFinite(value) || value < 0) {
            setThresholdInvalid(true)
            return
          }
          setThresholdInvalid(false)
          setDraftThresholds([...draftThresholds, value])
          setThresholdInput('')
        }
        const removeThreshold = (index) => {
          setDraftThresholds(draftThresholds.filter((_, i) => i !== index))
        }
        // Manual test: enqueue a demo alert so the toast can be seen without
        // waiting for a real balance drop. Uses the first configured threshold
        // (or a fixed value) and the current balance.
        const testAlert = () => {
          const currency = view.balance.currency || 'CNY'
          const amount = view.balance.amount || 0
          const threshold = draftThresholds.length > 0 ? draftThresholds[0] : 10
          setSnapshot({
            alerts: [{ id: 'test-' + Date.now(), threshold, amount, currency }]
          })
        }
        const draftRefreshSeconds = (candidate) => {
          const h = parseInt(candidate.h, 10)
          const m = parseInt(candidate.m, 10)
          const s = parseInt(candidate.s, 10)
          const parts = [h, m, s]
        ...
        const applyRefresh = (next) => {
          setDraft(next)
          setInvalid(draftRefreshSeconds(next) === null)
        }
        const applySettings = () => {
          const total = draftRefreshSeconds(draft)
          if (total === null) {
            setInvalid(true)
            return
          }
          setSnapshot({ fields: draftFields, refreshSeconds: total, thresholds: draftThresholds })
          writeSettings({ fields: draftFields, refreshSeconds: total, thresholds: draftThresholds }).then(() => readSettings()).catch((error) => {
        ...
        const dirty = JSON.stringify(draftFields) !== JSON.stringify(committedFields)
          || draftRefreshSeconds(draft) !== committedRefresh
          || JSON.stringify(draftThresholds) !== JSON.stringify(committedThresholds)
        const rows = []
        ...
        return react.createElement('div', { className: 'dyn-balance-settings' },
          ...
          react.createElement('div', { className: 'dyn-balance-refresh-block' },
            react.createElement('div', { className: 'dyn-balance-refresh-label' }, t('settings.alert')),
            react.createElement('div', { className: 'dyn-balance-refresh-hint' }, t('settings.alertHint')),
            draftThresholds.map((threshold, index) => react.createElement('div', { className: 'dyn-balance-field-row', key: index },
              react.createElement('span', { className: 'dyn-balance-field-name' },
                SYMBOL(view.balance.currency || 'CNY') + threshold.toFixed(2)),
              react.createElement('button', {
                type: 'button',
                className: 'dyn-balance-field-btn',
            ...
            react.createElement('div', { className: 'dyn-balance-alert-add-row' },
              react.createElement('input', {
                type: 'number',
                min: 0,
                step: 0.01,
              ...
              react.createElement('button', {
                type: 'button',
                className: 'dyn-balance-actions-btn',
              ...
              react.createElement('button', {
                ...
                className: 'dyn-balance-actions-btn',
                onClick: testAlert
              }, t('settings.alertTest'))),
            thresholdInvalid ? react.createElement('div', { className: 'dyn-balance-refresh-error' }, t('settings.alertInvalid')) : null),
